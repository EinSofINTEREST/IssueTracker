...
// retryTestPub 는 RetryScheduler 가 *bus.Publisher 의존성으로 변경된 후 (이슈 #390)
// 기존 mockProducer 검증 흐름을 유지하기 위한 thin helper 입니다.
// retryMockProducer 를 내부 producer 로 가진 실제 *bus.Publisher 를 생성합니다.
func retryTestPub(producer *retryMockProducer) *bus.Publisher {
...
func TestKafkaImmediateRetryScheduler_PublishesToPriorityTopic(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			sched := bus.NewKafkaImmediateRetryScheduler(retryTestPub(producer))
...
func TestRedisDelayedRetryScheduler_Enqueue_StoresJobAndLastErr(t *testing.T) {
	...
	sched := bus.NewRedisDelayedRetryScheduler(q, retryTestPub(prod), bus.DefaultRedisRetrySchedulerConfig(), retrySchedTestLogger())
...
func TestRedisDelayedRetryScheduler_Run_PublishesDueItems(t *testing.T) {
	q := newFakeRetryQueue()
	prod := new(retryMockProducer)
	cfg := bus.DefaultRedisRetrySchedulerConfig()
	cfg.PollInterval = 10 * time.Millisecond
	sched := bus.NewRedisDelayedRetryScheduler(q, retryTestPub(prod), cfg, retrySchedTestLogger())
...
func TestRedisDelayedRetryScheduler_DefaultConfigBoundary(t *testing.T) {
	q := newFakeRetryQueue()
	prod := new(retryMockProducer)
	sched := bus.NewRedisDelayedRetryScheduler(q, retryTestPub(prod), bus.RedisRetrySchedulerConfig{}, retrySchedTestLogger())
...
func TestRedisDelayedRetryScheduler_IdleHeartbeat_CompressedEveryN(t *testing.T) {
	...
	sched := bus.NewRedisDelayedRetryScheduler(q, retryTestPub(prod), cfg, captureDebugLogger(buf))
