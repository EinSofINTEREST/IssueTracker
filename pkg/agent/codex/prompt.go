...
var ErrRejectReasonPlaceholderMissing = errors.New("codex: prompt template missing " + rejectReasonPlaceholder + " placeholder (LLM_PROMPT_DIR override may need update)")

// buildPrompt 는 Codex 에 전달할 프롬프트를 생성합니다.
// sessionPath 는 컨테이너 내 세션 디렉토리 경로 (예: /workspace/<sessionID>) 입니다.
// Codex 는 해당 경로의 page.html 을 파일 읽기 툴로 직접 접근합니다.
//
...
func buildPrompt(loader prompt.Loader, host string, targetType model.TargetType, sessionPath, rejectReason string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("load codex prompt %q: %w", name, err)
	}
	...
	if rejectReason != "" && !strings.Contains(template, rejectReasonPlaceholder) {
		return "", fmt.Errorf("build codex prompt %q: %w", name, ErrRejectReasonPlaceholderMissing)
	}
...
func promptNameFor(targetType model.TargetType) string {
	if targetType == model.TargetTypeList {
		return "parser/codex/list.user"
	}
	return "parser/codex/page.user"
}
