...
const (
	...
	envWorkerCount     = "CODEX_WORKER_COUNT"
...
//
// codex 전용 옵션이 추가 필요한 경우 본 alias 를 별도 struct 로 분리하고 agent.PoolConfig 를
// embed 하면 됩니다. 현재는 Name 만 사용 — agent 공용 struct 를 그대로 노출.
//
// 우선순위 (예 Name="parser"):
//  1. PARSER_CODEX_WORKER_COUNT — stage 별 명시
//  2. CODEX_WORKER_COUNT — fallback
//  3. defaultWorkerCount
//
// 같은 정책이 CODEX_TIMEOUT / IMAGE / MODEL 등에도 적용 — agent.StageEnv 참조.
...
func NewPool(workers []*Worker, log *logger.Logger) (*Pool, error) {
	if log == nil {
		return nil, errors.New("codex: NewPool requires non-nil logger")
	}
	if len(workers) == 0 {
		return nil, errors.New("codex: NewPool requires at least 1 worker")
	}
	for i, w := range workers {
		if w == nil {
			return nil, fmt.Errorf("codex: NewPool workers[%d] is nil", i)
		}
	}
...
// 본 함수는 PoolConfig{Name: ""} 로 위임 — CODEX_* env 만 lookup.
...
// 변수가 우선 적용됩니다 — `<NAME>_CODEX_*` 이 있으면 사용, 없으면 `CODEX_*` fallback.
//
...
func NewPoolFromConfig(cfg PoolConfig, loader prompt.Loader, log *logger.Logger) (*Pool, error) {
	if log == nil {
		return nil, errors.New("codex: NewPoolFromConfig requires non-nil logger")
	}
	if loader == nil {
		return nil, errors.New("codex: NewPoolFromConfig requires non-nil prompt loader")
	}
	...
	for i := 0; i < count; i++ {
		...
		if err != nil {
			return nil, fmt.Errorf("codex: NewPoolFromConfig worker[%d]: %w", i, err)
		}
	...
	poolLog.WithFields(map[string]interface{}{
		"worker_count": count,
	}).Info("codex worker pool constructed (containers not started yet)")
	return pool, nil
}
...
func (p *Pool) Start(ctx context.Context) error {
	...
	for i, err := range errs {
		if err != nil && firstErr == nil {
			firstErr = fmt.Errorf("codex: pool worker[%d] start failed: %w", i, err)
		}
		if err != nil {
			failedAny = true
		}
	...
	}).Info("codex worker pool started (warm containers)")
	return nil
}
...
func (p *Pool) Stop(ctx context.Context) error {
	...
	for i, err := range errs {
		if err != nil {
			p.log.WithFields(map[string]interface{}{
				"worker_idx": i,
			}).WithError(err).Warn("codex pool worker stop failed")
			if firstErr == nil {
				firstErr = fmt.Errorf("codex: pool worker[%d] stop failed: %w", i, err)
			}
		}
	}
	if firstErr == nil {
		p.log.Info("codex worker pool stopped")
	}
	return firstErr
}
...
//
// enrich.Extractor 구현체 (extractor/codex.go) 가 호출하는 진입점.
