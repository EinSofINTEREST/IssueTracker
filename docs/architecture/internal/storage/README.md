...
│  pgx/v5 + TimedPool 데코  │  goredis client + Lua       │
...
|---|---|---|
| [`model/`](../../../../internal/storage/model/) | 도메인 모델 + 공유 타입 | `BlacklistRecord`, `BlacklistMode`, `ParserRuleRecord`, `SelectorMap`, `TargetType`, `ValidationStatus`, `BlacklistFilter`, `ParserRuleFilter` |
...
| [`postgres/`](../../../../internal/storage/postgres/) | PostgreSQL 구현 + `TimedPool` (이슈 #427) — [postgres.md](postgres.md) 참조 | `NewBlacklistRepository`, `NewParserRuleRepository`, `NewContentRepository`, ... + `TimedPool` 데코레이터 |
...
pgxpool 의 `Acquire()` 가 풀 고갈 시 **무한 블로킹** 되는 문제 — `internal/storage/postgres/` 의 `TimedPool` 데코레이터가
모든 메소드 진입에 query-level timeout 을 적용. `cmd/issuetracker.go` 가 `Load()` 결과의 `QueryTimeout` 으로 wrap.
...
| [model/filter.go](../../../../internal/storage/model/) | `*Filter` 타입 — limit/offset/where 조건 빌더 |
...
- 이슈 #427 — TimedPool 데코레이터 (pgxpool Acquire 무한 블로킹 차단)
