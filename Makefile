...
pg-start: ## 로컬 PostgreSQL 연결 확인 (DEV: 시스템 postgres 사용)
	...
	@pg_isready -h $${DB_HOST:-localhost} -p $${DB_PORT:-5432} -U $${DB_USER:-postgres} || \
	  (echo "ERROR: Local PostgreSQL is not running." && \
	   ...
	   exit 1)
	@echo ""
	@echo "  PostgreSQL → $${DB_HOST:-localhost}:$${DB_PORT:-5432}"

pg-stop: ## (DEV: 로컬 postgres는 시스템에서 직접 관리)
	@echo "Local PostgreSQL is managed by the system. Use your service manager to stop it."

pg-clean: ## (DEV: 로컬 postgres는 시스템에서 직접 관리)
	@echo "Local PostgreSQL is managed by the system. Use your service manager to manage data."
...
pg-status: ## 로컬 PostgreSQL 연결 상태 확인
	@pg_isready -h $${DB_HOST:-localhost} -p $${DB_PORT:-5432} -U $${DB_USER:-postgres}

pg-psql: ## 로컬 PostgreSQL psql 접속
	@psql \
	  -h $${DB_HOST:-localhost} \
	  -p $${DB_PORT:-5432} \
	  -U $${DB_USER:-postgres} \
	  -d $${DB_NAME:-ecoscrapper}
