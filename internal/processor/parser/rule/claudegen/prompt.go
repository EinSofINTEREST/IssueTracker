...
func buildPrompt(host string, targetType storage.TargetType, sessionPath string) string {
	fieldsGuide := fieldsGuideFor(targetType)
	return fmt.Sprintf(`Read the HTML file at %s/page.html from the %s website.
...
Return ONLY the JSON object.`, sessionPath, host, string(targetType), fieldsGuide)
}
