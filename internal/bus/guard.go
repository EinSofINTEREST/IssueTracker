...
// 만 알고 싶어합니다. worker 패키지의 IngestionMarker 와 method signature 가 동일하지만
// publisher 가 worker 를 import 하지 않도록 별도 정의 — RedisIngestionMarker 등 기존
...
// ingestionMarkerRef 는 atomic.Pointer 가 인터페이스 값을 직접 저장하지 못하므로
// IngestionMarker 인터페이스를 감싸 atomic 교체를 지원하는 wrapper 입니다.
type ingestionMarkerRef struct {
	l IngestionMarker
}
...
func (p *Publisher) SetIngestionMarker(l IngestionMarker) {
	...
	p.lock.Store(&ingestionMarkerRef{l: l})
}

// SetPipelineGuard 는 publish 진입 시 target type 별 정책으로 marker 를 잡을 PipelineGuard 를
// 설정합니다.
//
// guard 가 설정되어 있으면 IngestionMarker 보다 우선 — 모든 target type 에 적용 (Article 24h,
// Category 단명 TTL). nil 전달 시 가드 비활성 — IngestionMarker fallback (있으면).
