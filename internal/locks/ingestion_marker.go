...
//     구분 가능
type RedisIngestionMarker struct {
	locker redisIngestionMarker
	ttl    time.Duration
}
...
func NewRedisIngestionMarker(locker redisIngestionMarker, ttl time.Duration) *RedisIngestionMarker {
	if ttl <= 0 {
		ttl = DefaultIngestionMarkTTL
	}
	return &RedisIngestionMarker{locker: locker, ttl: ttl}
}

// Acquire 는 URL 의 진입 marker 를 SET NX EX 로 atomic 시도합니다.
//
// nil receiver / nil locker 보호: zero-value RedisIngestionMarker 또는 NewRedisIngestionMarker(nil, ...)
// 호출 시에도 panic 없이 에러 반환 — 호출자가 fail-open / 알림 등 처리 가능.
func (l *RedisIngestionMarker) Acquire(ctx context.Context, url string) (bool, error) {
...
//
// ttl<=0 이면 RedisIngestionMarker 의 default TTL 사용. nil receiver / nil locker 보호는 Acquire 와 동일.
func (l *RedisIngestionMarker) AcquireWithTTL(ctx context.Context, url string, ttl time.Duration) (bool, error) {
	if l == nil || l.locker == nil {
		return false, errors.New("ingestion lock locker is nil")
	}
	if ttl <= 0 {
...
func (l *RedisIngestionMarker) Invalidate(ctx context.Context, url string) error {
	if l == nil || l.locker == nil {
...
type NoopIngestionMarker struct{}

// Acquire always returns acquired=true (Noop).
func (NoopIngestionMarker) Acquire(_ context.Context, _ string) (bool, error) { return true, nil }

// AcquireWithTTL always returns acquired=true (Noop) — TTL 인자는 무시.
func (NoopIngestionMarker) AcquireWithTTL(_ context.Context, _ string, _ time.Duration) (bool, error) {
	return true, nil
}

// Invalidate is a no-op.
func (NoopIngestionMarker) Invalidate(_ context.Context, _ string) error { return nil }

// 컴파일 타임 인터페이스 만족 검증.
var (
	_ IngestionMarker = (*RedisIngestionMarker)(nil)
	_ IngestionMarker = NoopIngestionMarker{}
)
